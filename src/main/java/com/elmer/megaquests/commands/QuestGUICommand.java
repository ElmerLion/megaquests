...
public class QuestGUICommand implements CommandExecutor {
    ...
    public void createQuestGUI(Player player){
        ...
        for (Quests quests : Quests.values()){
            ...
            megaQuests.getQuestManager().completedQuests.clear();
        }
        ...
        if (availableQuests.length >= 5) {
            ...
            for (int i = startingSlot; i <= endingSlot; i++){
                ...
                Map<Quests, Integer> taskAmountMap = megaQuests.getQuestManager().getPlayerTaskAmountsMap().getOrDefault(playerId, new HashMap<>());
                taskAmountMap.put(quest, taskAmount);
                megaQuests.getQuestManager().getPlayerTaskAmountsMap().put(playerId, taskAmountMap);
